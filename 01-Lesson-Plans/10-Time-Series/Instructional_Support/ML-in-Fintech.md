...
Insurance companies, for example, attempt to lower cost by putting NLP-driven chatbots into the insurance claim process, so that the long process of filing a claim, presenting evidence via uploaded photos, and settling payments are done largely without the help of a specialist “claims adjuster”. Yet along with the chatbots, ML is used throughout this process to flag potentially fraudulent claims or a dissatisfied customer.

Consider again fintech’s role in dispensing financial advice and overseeing savings. NLP can replace the (traditionally human) financial advisor by asking questions pertaining to tolerated risk, ability to save, or when they want to retire. For  banks looking to automate the above, they’d likely involve an NLP chatbot to extract the above answers from the potential customer, and another machine learning model (Random Forests, KNN, or a DNN) behind the scenes. This latter model would correlate data on the bank’s past customers with the various financial products those customers primarily chose. The bank would then use that ML model to infer those statistical relationships. The NLP chatbot would then use those findings in order to allow it to inform new customers about what types of financial products they might be interested in.
