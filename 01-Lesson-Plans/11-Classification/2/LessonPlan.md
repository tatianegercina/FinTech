...
* One of the key differences between **boosting** and **bagging** is that **boosting** algorithms will weigh predictions based on accuracy, and as long as data points are weighted as inaccurate, **boosting** algorithms will continue to run. Instead of weighing predictions, **bagging** algorithms resample and replace data in order to improve model fitting and accuracy.
  ...
  * Summarize the comparison again to help reinforce the differences:
    ...
    * Bagging combines predictions from multiple models, taking the highest ‘voted’ prediction among the models as the most likely prediction. Bagging resamples and replaces data in order to improve model fitting and accuracy.

    * Boosting also iteratively resamples and replaces data in order to train the best model. The difference is that boosting resamples with greater frequency those samples that previously had the highest error.
...
* Ask if there is a volunteer who would like to summarize the difference between boosting and bagging algorithms.
  ...
  * **Answer** Bagging averages predictions from multiple models or samples. Boosting is similar, but iteratively resamples (resampling those observations with higher error rates) for each model, before averaging model predictions in order to train the best overall model.
...
* KNN (which we'll cover in coming weeks) naturally supports incremental learning (data streams).
