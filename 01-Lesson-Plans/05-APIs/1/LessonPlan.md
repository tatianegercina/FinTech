...
For the past three weeks, students have been immersed in the world of Python and standard Python libraries that are included in the Anaconda environment (i.e., Pandas and Matplotlib). They've also learned some critical techniques for analyzing financial data, such as calculating daily returns and risk-reward ratios. It's now time for students to take it to the next level and enter into the world of APIs. There are several FinTech APIs available that will allow students to perform more robust analytics. They will also expose students to datasets they would not typically be able to access or consolidate easily. Now, students will learn the basics of APIs and API requests so that they can begin incorporating APIs into their work.
...
Before students leave at the end of the class, instruct them to sign up for the APIs that are going to be used tomorrow. These APIs will be used in the next lesson, and Lesson 5.2 activities will require students to have API keys. The links are provided below. Remember to slack the links to the students, so they have a resource to sign up with.
