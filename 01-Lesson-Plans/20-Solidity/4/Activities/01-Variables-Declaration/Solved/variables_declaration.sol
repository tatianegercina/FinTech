...
contract TravelExpenses {
  ...
  // 2. Create payable addess variable named `personal_account` and assign a valid Ethereum address to it
  ...
  function RecordExpense(uint32 new_net_expense_usd, uint tax_rate) public {
    ...
    // 12. Create an usigned `256 bit` integer `taxes_usd` and assign the taxes in USD from `new_net_expense_usd`.
