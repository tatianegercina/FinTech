...
def create_spreadsheet(lesson_plan_path,
    ...
    with xlsxwriter.Workbook(output_path) as workbook:
        # Create worksheet
        worksheet = workbook.add_worksheet("Weeknight")
        # Zoom in
        ...
        for number, activity, duration, priority in read_md(lesson_plan_path,
                                                  morning=morning):
            # import logging
            # logging.warning(f"{type(duration)}")
            if "Structured" not in activity and "Career" not in activity:
        ...
        morning=True
        # Create worksheet
        worksheet = workbook.add_worksheet("Weekend")
        # Zoom in
        worksheet.set_zoom(150)
        # Merge cells for title
        cell_format = workbook.add_format({'align': 'center', 'bold': True})
        ...
        # Write last row
        if "Structured" not in activity and "Career" not in activity:
            activity = "Instructor Do: Structured Review"
            duration = "0:35"
