...
Today's class introduces the concepts that underlay neural networks, a class of machine learning algorithms that, like other ML techniques, can be used for classification and regression, but are perhaps uniquely able to learn very complex, non-linear models. Conceptually, neural nets were inspired by neurons in the human brain, and have a similar property of scaling to meet the complex environments that they operate in. 

Neural networks are very flexible in their applications, but they are not necessarily meant to be "plug-and-play." From preprocessing input data to constructing a neural network architecture, students will need a fair bit of background in order to successfully use neural networks. Today's class will begin with the most basic unit of a neural network, the neuron, and guide students through constructing "shallow" neural networks with the help of a graphical interface. 
