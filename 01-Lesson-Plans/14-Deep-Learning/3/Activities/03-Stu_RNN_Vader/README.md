...
You will use the values from the `pos` polarity score to plot the ROC curve; we will consider the `pos` score as the probability of a review comment to be positive. To plot the ROC curve, these probabilities should range from `0` to `1`, so the values of the `y_vader_prob` list will be normalized using [min-max normalization](https://en.wikipedia.org/wiki/Feature_scaling#Rescaling_(min-max_normalization)). You may save the resulting normalized values in a variable called `y_vader_prob_norm`.
...
Review all the metrics you compute, evaluate the ROC curve plots, and the AUC values to answer the following question:
