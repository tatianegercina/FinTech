...
def build_dashboard(data, signals):
    ...
    signals_plot = (
        signals[signals["entry/exit"] == 1.0].hvplot.scatter(y='sma10', marker='^', size=200, c='g', label='buy', padding=.1) *\
        signals[signals["entry/exit"] == -1.0].hvplot.scatter(y='sma10', marker='v', size=200, c='r', label='sell', padding=.1) *\
        signals.hvplot.line(y='sma10', label='sma10') *\
        signals.hvplot.line(y='sma20', label='sma20')
    )
    dashboard = pn.Column(
        '# Super Ninja Nano Micro Algo Trader 9001',
        data.hvplot(title="prices"),
        signals_plot.opts(title="signals plot", show_legend=False),
        "### Signals Table",
        signals.hvplot.table(title='signals table', columns=['close', 'entry/exit', 'sma10', 'sma20'], backlog=10)
        )
    return dashboard
