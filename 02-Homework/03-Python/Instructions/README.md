# Unit 3 | Assignment - Automate Your Day Job with Python

## Background

Well... you've made it!

It's time to put away the Excel sheet and join the big leagues. Welcome to the world of programming with Python. In this homework assignment, you'll be using the concepts you've learned to complete **2** Python activities, PyBank and PyRamen. Both of these activities encompasses a real-world situation where your newfound Python scripting skills can come in handy. These activities are far from easy so expect some hard work ahead!

### Before You Begin

1. Create a new GitHub repo called `python-homework`. Then, clone it to your computer.

2. Inside your local git repository, create a directory for both of the Python activities. Use folder names corresponding to the activities: **PyBank** and **PyRamen**.
...
* In this activity, you are tasked with creating a Python script for analyzing the financial records of your company. You will give a set of financial data called [budget_data.csv](PyBank/Resources/budget_data.csv). The dataset is composed of two columns: `Date` and `Profit/Losses`. (Thankfully, your company has rather lax standards for accounting so the records are simple.)
...
And if you're feeling particularly confident:
...
First things first, you'll need to read in your `menu_data.csv` as a list object so that you can cross-reference your menu data to your sales data as you read in your sales data in the coming steps.
...
Now that you've read in your `menu_data.csv`, set up the same process to begin reading the `sales_data.csv`; however, unlike how you merely appended every row of the menu data to a new list object, for the sales data you will need to perform conditionals to evaluate every row of the sales data and 'group' the sales data in a new `report` dictionary (in this case it will be a dict of dicts).

### Manipulate the Data

In order for you to generate a `report` dictionary that showcases on a per product basis...
...
* Create the following functions that take in the report and field of choice as parameters, and returns the calculations for that particular field of all keys in the report.
  * `sum_field(report, field)` - summarize up the values of the specified field for every item in the report
...
* As an example, your report details should look similar to the one below:
